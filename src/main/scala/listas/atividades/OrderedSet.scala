...
class OrderedSet[A](val comp: (A, A) => Int) {
    ...
    */
    def intersect(set1: List[A], set2: List[A]): List[A] = (set1, set2) match
        case (Nil, _) => Nil
        case (_, Nil) => Nil
        case (h1 :: t1, h2 :: t2) =>
            comp(h1, h2) match
                case result if result < 0 => intersect(t1, set2)
                case 0 => h1 :: intersect(t1, t2)
                case _ => intersect(set1, t2)
