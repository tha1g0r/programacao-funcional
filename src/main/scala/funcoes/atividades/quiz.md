...
* [x] Trata-se de um sistema formal baseado em variáveis, abstrações de funções lambda e aplicação de funções, sendo equivalente em poder de computação a uma Máquina de Turing.
...
* [x] A palavra-chave `return` é geralmente desencorajada em programação funcional.
* [x] O tipo do resultado produzido pode ser inferido pelo compilador, mas é boa prática especificá-lo.
* [x] O corpo da função pode ser uma expressão única ou um bloco de código.
...
* [x] O código produzirá o quadrado apenas dos números pares da lista.
* [ ] As funções `f1` e `f2` são closures porque capturam o parâmetro `data`.
* [ ] As funções aninhadas não podem acessar o parâmetro `data`.
* [x] O código utiliza funções aninhadas.
* [x] As funções `f1` e `f2` são visíveis apenas dentro de `processData`.

## Questão 4

Sobre as aplicações das lambdas (funções anônimas), assinale as alternativas corretas:

* [x] Podem ser passadas como argumentos para métodos de objetos.
* [x] Podem ser usadas para criar closures.
* [x] Permitem criar funções de ordem superior.
* [ ] Só podem ser usadas em coleções.
* [x] São úteis para tratamento de eventos e callbacks.
...
* [x] São literais, podendo ser atribuídas a variáveis.
* [ ] Devem sempre ter o tipo do resultado explicitamente declarado.
* [x] São úteis para funções de ordem superior.
* [x] Podem ser passadas como argumentos para outras funções.
...
* [x] Uma função que recebe uma string e uma função, e produz um booleano.
...
* [x] Uma função que recebe como argumento uma função e produz uma string.
...
* [x] Uma função de ordem superior é aquela que recebe pelo menos uma outra função como argumento, ou produz uma função como resultado, ou ambos.
...
* [x] Podem receber funções como argumentos.
* [x] Podem produzir funções como resultado.
...
* [x] Reduz uma coleção a um único valor usando uma função de agregação.
* [x] Utiliza o primeiro elemento da coleção como valor inicial do acumulador.
* [ ] Produz sempre um valor do tipo `Option`.
* [x] Lança uma exceção se aplicado a uma lista vazia.
...
* [x] Pode produzir uma coleção menor que a original.
* [ ] Sempre produz uma coleção do mesmo tamanho que a original.
* [x] Não modifica a coleção original.
* [x] Recebe como argumento um predicado.
...
* [x] Não modifica a coleção original, produzindo uma nova coleção.
...
* [x] Preserva o tamanho da coleção original.
...
* [x] A composição de funções é associativa, o que significa que expressões como `(f andThen g) andThen h` e `f andThen (g andThen h)` produzem o mesmo resultado comportamental, embora as assinaturas intermediárias de tipo devam ser compatíveis.
...
val g = (x: Int) => x * 3
val h = (x: Int) => x * x
val p = f andThen g compose h // (f andThen g) compose h => g(f(h(x)))
val result = p(2)
```
...
* [x] Uma função que captura variáveis do ambiente léxico onde foi definida.
